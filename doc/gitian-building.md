...
- [Building Proton Core](#building-growth-core)
...
git clone https://github.com/growthcore/growthcoin
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/growth/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/growthcore/growthcoin
...
For example:
```bash
URL=https://github.com/crowning-/growth.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit growth=${COMMIT} --url growth=${URL} ../growth/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit growth=${COMMIT} --url growth=${URL} ../growth/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit growth=${COMMIT} --url growth=${URL} ../growth/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the growth git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../growth/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/growthcore/growthcoin-detached-sigs.git

BTCPATH=/some/root/path/growthcoin.git
SIGPATH=/some/root/path/growthcoin-detached-sigs.git

./bin/gbuild --url growth=${BTCPATH},signature=${SIGPATH} ../growth/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/growth-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/growth-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/growth-osx-build.assert
```
...
[growth/gitian.sigs](https://github.com/growthcoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
