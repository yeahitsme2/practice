...
const char * const BITCOIN_CONF_FILENAME = "growth.conf";
const char * const BITCOIN_PID_FILENAME = "growthd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "growth" is a composite category enabling all Growth-related debug output
            if(ptrCategory->count(string("growth"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GrowthCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GrowthCoin
    // Mac: ~/Library/Application Support/GrowthCoin
    // Unix: ~/.growthcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GrowthCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GrowthCoin";
#else
    // Unix
    return pathRet / ".growthcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty growth.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override growth.conf
